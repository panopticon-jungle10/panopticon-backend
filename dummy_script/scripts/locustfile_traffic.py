"""
Locust Performance Test for Producer Server - Traces Only

Goal: Generate high-volume trace traffic only

Configuration:
- Each request sends 30 spans total:
  - 30 spans to /dummy/traces (3 traces × 10 spans each)
...
TRACES_PER_REQUEST = 3  # Each request contains 3 complete traces
TOTAL_ITEMS_PER_REQUEST = TRACES_PER_REQUEST * SPANS_PER_TRACE  # 30 total items
...
def generate_batch_spans():
    """
    Generate a batch of 30 spans (3 traces × 10 spans each).

    Each trace is independent with its own trace_id and timestamp.

    Returns:
        list: 30 span dictionaries from 3 independent traces
    """
    all_spans = []

    # Generate 3 complete traces
...
class HighVolumeTrafficTest(HttpUser):
    """
    High-volume traces load test for producer server.

    Each user sends only traces:
    - 30 spans (3 traces × 10 spans) to /dummy/traces
    - Total: 30 items per batch
    ...
    def send_batch_spans(self):
        """Send 30 spans (3 traces × 10 spans) to /dummy/traces endpoint"""
        global request_count

        # Generate batch of 30 spans
        ...
        with self.client.post(
            ...
            json=batch_spans,
            catch_response=True,
            name="POST /dummy/traces (30 spans)",
        ) as response:
...
def on_test_start(environment, **kwargs):
    ...
    if not isinstance(environment.runner, MasterRunner):
        ...
        print("\n" + "=" * 80)
        print("🚀 High-Volume Traces Load Test Starting")
        print("=" * 80)
        print(f"Target Server: {PRODUCER_URL}")
        print(f"Test Endpoints:")
        print(f"  - POST /dummy/traces (30 spans per request)")
        print(f"\nData per Request:")
        print(
...
def on_request(
    ...
    if request_count % 1000 == 0 and request_count > 0:
        ...
        print(
            f"Progress: {request_count:,} batches ({total_spans:,} spans sent)"
        )
...
def on_test_stop(environment, **kwargs):
    ...
    if not isinstance(environment.runner, MasterRunner):
        ...
        print("\n" + "=" * 80)
        print("✅ High-Volume Traces Load Test Completed")
        print("=" * 80)
        ...
        if stats.total_rps > 0:
            actual_items_per_sec = stats.total_rps * TOTAL_ITEMS_PER_REQUEST
            print(f"\nPerformance:")
